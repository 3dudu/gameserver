...
public class IceBirdH5Handler {
    static Logger logger = Logger.getLogger(IceBirdH5Handler.class);

    @Cmd("/p/IceBirdH5PayNotify/cb")
    public void IceBirdH5PayNotify(User sender, HttpRequestJSONObject params) {
        String json = null;
        if (!params.isEmpty()) {
        ...
        //检验参数是否匹配
        Constants.PlatformOption.IceBirdH5 iceBirdH5 = GlobalCache.getPfConfig("IceBirdH5");
        //检验gameId是否配置
        if(!iceBirdH5.game_id.equals(notifyData.gameId)){
            String cause ="冰鸟H5 参数未配置,gameId="+ notifyData.gameId;
            logger.info(cause);
            sender.sendAndDisconnect(MsgFactory.getDefaultErrorMsg("gameId未配置"));
            return;
        }
        //检验app_secret是否配置
        String app_secret = iceBirdH5.app_secret;
        if (null==app_secret) {
...
//        202b962234w4ers2aa
        ...
        if (!notifyData.sign.equals(signStr)) {
            logger.info("验签失败!冰鸟H5 sign="+notifyData.sign+"==============="+"32位小写后signStr:"+signStr);
        ...
        payTrade.setPlatform("IceBirdH5");
        payTrade.setStatus(2);
        payTrade.updateToDB();
        //发钻石
        boolean isSychnSuccess = PaymentToDeliveryUnified.delivery(payTrade, "IceBirdH5");
