...
public class WanXinKeYiSDKServiceHandler {

    static Logger logger = Logger.getLogger(WanXinKeYiSDKServiceHandler.class);

    @Cmd("/p/WanXinKeYiH5PayNotify/cb")
    public void WanXinH5PayNotify(User sender, HttpRequestJSONObject params) {
        ...
        logger.info("WanXinKeYiSDKNotifyData:" + notifyData.toString());
        //检验参数是否匹配
        Constants.PlatformOption.WanXinKeYi wanXinKeYi = GlobalCache.getPfConfig("WanXinKeYi");
        String cause = "";
        //检验gameId是否配置
        if (null == wanXinKeYi.apps.get(notifyData.tp)) {
            logger.info("玩心-可以 参数未配置,channel_code=" + notifyData.tp);
            sender.sendAndDisconnect(YueWanUtils.getErrorMsg("玩心-可以 参数未配置,channel_code=" + notifyData.tp, 0, ""));
            return;
        }
        //检验app_secret是否配置
        String xx_pay_sign = wanXinKeYi.apps.get(notifyData.tp).xx_pay_sign;
        if (null == xx_pay_sign) {
        ...
        payTrade.setPlatform("WanXinKeYi");
        payTrade.setStatus(2);
        payTrade.updateToDB();
        //发钻石
        boolean isSychnSuccess = PaymentToDeliveryUnified.delivery(payTrade, "WanXinKeYi");
