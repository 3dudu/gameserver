 ...
 */
public class YDSDKAppleService extends AbsLoginService {
    ...
    public UserPo loginUser(String[] parameters, JSONObject rs) {
        Constants.PlatformOption.YDSDKApple config = getConfig();
        if (parameters.length == 4) {
            String channel = parameters[0];
            String uid = parameters[2];
            String appId = parameters[3];
            if (config.apps.get(channel).appId == null) {
                logger.info("优点平台(ios) 参数未配置,appId=" + appId);
                return null;
            }
            String openkey = config.apps.get(channel).openkey;
            if (openkey == null) {
            ...
            logger.info("优点SDK(ios) post:" + verify_url + ",param=" + postData + ",返回=" + result);
            if (result != null) {
