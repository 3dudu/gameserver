...
public class IceBirdH5Service extends AbsLoginService{
    ...
    public UserPo loginUser(String[] parameters, JSONObject rs) {
        ...
        //取配置参数
        Constants.PlatformOption.IceBirdH5 config = getConfig();
        if (parameters.length == 7) {
            ...
            if (!config.game_id.equals(gameId)) {
                logger.info("冰鸟H5 参数未配置,gameId=" + gameId);
                return null;
            }
            ...
            logger.info("冰鸟H5 post:" + requestUri + ",param=" + postData + ",result=" + request);
            if (request != null) {
