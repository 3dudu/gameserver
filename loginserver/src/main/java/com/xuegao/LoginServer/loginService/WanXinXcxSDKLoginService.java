...
public class WanXinXcxSDKLoginService  extends  AbsLoginService{

    private static final String requestUri = "https://japi.hzwxbz3.cn/oauth/token";


    @Override
    public UserPo loginUser(String[] parameters, JSONObject rs) {
        //取配置参数
        Constants.PlatformOption.WanXinXcxSDK config = getConfig();
        if (parameters.length == 4) {
            String channel = parameters[0]; //咱们的channel_code
            String pf = parameters[1];  //WanXinXcxSDK
            ...
            logger.info("玩心-小程序 签名字符拼接为：" + signStr +",md5签名为：" + sign);
            ...
            data.remove("j_game_secret");
            data.put("sign",sign);
            String request = null;
            try {
                request = RequestUtil.requestWithPost(requestUri,new HashMap() ,JSONObject.toJSONString(data));
                logger.info("玩心-小程序 post:" + requestUri + ",param=" + JSONObject.toJSONString(data) + ",result=" + request);
                if (request != null) {
