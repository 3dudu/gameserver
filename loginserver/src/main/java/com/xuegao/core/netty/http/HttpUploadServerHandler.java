...
//			ReferenceCountUtil.release(msg);
	...
	public ChannelFuture sendMsg(User user, Object rs) {
        ...
        boolean close = true;
        if(request != null){
            close = request.headers().contains(CONNECTION, HttpHeaders.Values.CLOSE, true)
    ...
    public void userEventTriggered(ChannelHandlerContext ctx, Object evt) throws Exception {
        ...
        if (evt instanceof IdleStateEvent) {
            ...
            if (e.state() == IdleState.READER_IDLE) {
                logger.info("客户端10分钟无消息发出，连接断开!"+ctx.channel().toString());
                ctx.close();
            }else if(e.state() == IdleState.WRITER_IDLE) {
                logger.info("服务端10分钟无消息发出，连接断开!"+ctx.channel().toString());
                ctx.close();
            }
        }
    }
