...
public class JedisUtil {
	...
	private static Map<String, JedisUtil> jedisUtilMap=new HashMap<String,JedisUtil>();
	...
	public static void reloadConfig(){
		try {
			File f=ClassPathUtil.findFile("redis.properties");
			if(f==null){
				logger.error("redis.properties not found!");
			}else {
				logger.info("开始创建redis连接池...-->配置文件地址:"+f.getPath());
				initialPool(f.getPath());
			}
		} catch (Exception e) {
	 ...
	 * @return JedisPool
	 */
	public static void initialPool(String confPath) {
		jedisUtilMap.clear();
		HashMap<String, String> redisPropMap = FileUtil.readFile(confPath,
				new HashMap<String, String>(), "UTF-8");
		// 池基本配置
		Map<String, Map<String, String>> map=new HashMap<String, Map<String,String>>();
		for(String s:redisPropMap.keySet()){
			if(s.trim().startsWith("#")){
				continue;
			}
			String v=redisPropMap.get(s);
			String k1=StringUtil.getMiddleString(s, "redis", ".");
			if(null==k1){
				continue;
			}
			String k2=s.substring(s.indexOf(".")+1);
			Map<String, String> cmap=map.get(k1);
			if(cmap==null){
		...
		for(String index:map.keySet()){
			Map<String, String> props=map.get(index);
			String alias=props.get("alias");
			String host = props.get("host");
			String rport = props.get("port");
			String maxIdle = props.get("maxIdle");
			String maxActive = props.get("maxActive");
			String maxWait = props.get("maxWait");
			String testOnBorrow = props.get("testOnBorrow");
			String password=props.get("password");
			logger.info("redis"+index+"--->alias="+alias+",host="+host+",port="+rport+",maxidle="+maxIdle+",maxActive="+maxActive+",maxWait="+maxWait+",testOnBorrow="+testOnBorrow+",password="+password);
			int port = rport == null ? 6379 : Integer.parseInt(rport);
			int maxidle = maxIdle == null ? 200 : Integer.parseInt(maxIdle);
			int maxactive = maxActive == null ? 500 : Integer.parseInt(maxActive);
			int maxwait = maxWait == null ? 10000 : Integer.parseInt(maxWait);
			boolean testonborrow = testOnBorrow == null ? false : Boolean
					.parseBoolean(testOnBorrow);
			if(host==null||alias==null){
				logger.error("---------------redis配置错误:缺少必要的host和alias。 index="+index+"-------------");
				continue;
			}
			...
			if(password!=null&&password.trim().length()>0){
				jpool = new JedisPool(config, host, port,30*1000,password);
			}else{
				jpool = new JedisPool(config, host, port,30*1000);
			}
			jpool.returnResource(jpool.getResource());
			logger.info("Redis Pool---->"+host+":"+port);
			//放入 jedisutil map
			JedisUtil jedisUtil=new JedisUtil(jpool);
			jedisUtilMap.put(alias, jedisUtil);
		}
	}
	 ...
	 * @return
	 */
	public static JedisUtil getInstance(String alias) {
		return jedisUtilMap.get(alias);
	}
	...
	public class Lists {
		...
		public String blpop(String key){
			Jedis jedis = getJedis();
			List<byte[]> value = jedis.blpop(SafeEncoder.encode(key));
			if(value.size()==1){
				byte[] bs=value.get(0);
				returnJedis(jedis);
		...
		public long lmpush(String key, String[] valueArr) {
			Jedis jedis = getJedis();
			long count =-1;
			count = jedis.lpush(key,valueArr);
			returnJedis(jedis);
			return count;
		}
		public long rmpush(String key, String[] valueArr) {
			Jedis jedis = getJedis();
			long count =-1;
			count = jedis.rpush(key, valueArr);
			returnJedis(jedis);
			return count;
		}
