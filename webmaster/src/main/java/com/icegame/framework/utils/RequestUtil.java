...
//		}
	...
	public static List<String> uploadFiles(String url,List<File> files,String language,String md5,boolean openMd5) throws IOException{
		String BOUNDARY = "---------7d4a6d158c9"; // 定义数据分隔线
		URL u = new URL(url);
		HttpURLConnection conn = (HttpURLConnection) u.openConnection();
		// 发送POST请求必须设置如下两行
		...
		for(int i=0;i<leng;i++){
			...
			sb.append(BOUNDARY);
			sb.append("\r\n");
			sb.append("Content-Disposition: form-data;name=\"file"+i+"\";Language=" + language + ";md5=" + md5 + ";openMd5=" + openMd5 + ";filename=\""+ file.getName() + "\"\r\n");
			sb.append("Content-Type:application/octet-stream\r\n\r\n");
			...
			/**
			 * 传输耽搁的文件的时候一定要注释掉这个，不然会在多出一些数据，哇，这个问题几乎耽误了我一下午
			 */
			//out.write("\r\n".getBytes()); //多个文件时，二个文件之间加入这个
			in.close();
		}
	 ...
	 * @return
	 */
	public static String request3TimesWithPost(String url,byte[] bs) throws IOException {
